...
use App\Http\Controllers\Api\Signal\AccessController;
...
class RegisterController extends Controller
{
    ...
    public function createSendToken(Request $request, $consumer)
    {
        ...
        if($url == "email")
        {
            ...
            if(app('ExtVerificationTypeSystem') == 0)
            {
                MailManager::sendMail($changeRequest->email_new, ChangeEmail::class,
                    $consumer);
        ...
        if($url == "password")
        {
            $operation = SystemOperation::where('sys_oper_code', 'PasswordRecovery')->select('id')->first()->id;
            //            $operation = SystemOperation::where('sys_oper_code', 'PasswordChange')->select('id')->first()->id;
            MailManager::sendMail($changeRequest->email_issue, ChangePassword::class,
                $consumer);
   ...
   */
    public function verify($token)
    {
        ...
        if($verify->systemOperations->sys_oper_code == 'PasswordRecovery')
        {
            //проверяем актуальность токена при верификации
            $verifyConsumer = ConsumerActivityToken::where('token', $token)->where('status_n', $this->failed)
            ...
            if(isset($verifyConsumer))
            {
                ...
                if($consumer->consumer_status_n == $this->active)
                {
                    ...
                    return response()->json([
                        "error"          => false,
                        "message"        => "Установите новый пароль",
                        "reset_password" => true,
                        "reset_token"    => $token
                    ]);
                }
                else
                {
                    return response()->json([
                        "error"   => true,
                        "message" => "Установка нового пароля запрещена"
                    ], 400);
                }
            }
            else
        ...
        if(isset($verifyConsumer))
        {
            ...
            if($consumer->consumer_status_n == $this->sendRequest || $consumer->consumer_status_n == $this->issued)
            {
                ...
                $jwt_token = JWTAuth::fromUser($consumer);
        ...
        {
            $status = $texts->where('caption_name', "YouHaveCompletedRegistration")->first()->caption_translation;
            return response()->json([
                "message" => $status
            ], 400);
        }
   ...
   */
    public function confirmChangePassword(Request $request)
    {
        ...
        if(isset($verifyConsumer))
        {
            ...
            $repeat_password = $request['confirm_password'];
            $consumer = $verifyConsumer->consumer;
            if($password == $repeat_password)
            {
                $consumer->password = bcrypt($password);
                $consumer->password_changed_date = Carbon::now()->format("d-m-Y H:i:s");
                $consumer->save();
                $verifyConsumer->status_n = $this->success;
                $verifyConsumer->save();
